...
from billing.models import Billing, BillingDesk, Customer
...
def start_payment(request):
    ...
    payment = client.order.create({"amount": 100.00, 
                                   "currency": "INR", 
                                   "payment_capture": "1"})
    ...
    # function
    billing_desk = BillingDesk.objects.all()[0]
    customer = Customer.objects.all()[0]
    order = Billing.objects.create(
    ...
    serializer = BillingSerializer(order)
...
def handle_payment_success(request):
    ...
    order = Billing.objects.get(transaction_id=ord_id)
